...
class VA_Server:
    ...
    def __init__(self, job_config):
        ...
        # 若指定了训练 checkpoint（仅含 transformer），则从 ckpt 加载 transformer，其余从 base 加载
        base_path = job_config.wan22_pretrained_model_name_or_path
        transformer_path = getattr(
            job_config, "transformer_checkpoint_path", None
        ) or os.path.join(base_path, "transformer")
        ...
        if self.env_type in ('robotwin_tshape', 'libero'):
            vae_half = load_vae(
                os.path.join(job_config.wan22_pretrained_model_name_or_path,
                             'vae'),
                torch_dtype=self.dtype,
    ...
    def _encode_obs(self, obs):
        # env_type: robotwin_tshape = 3 视角；libero = 2 视角。VAE 按 3 视角设计，libero 需在相机维 pad 成 3 再编码。
        images = obs['obs']
        ...
        for k_i, k in enumerate(self.job_config.obs_cam_keys):
            if self.env_type == 'robotwin_tshape':
                ...
                # 2 views (image + wrist_image), same resolution
                height_i, width_i = self.height, self.width
            else:
                height_i, width_i = self.height, self.width
        ...
        if self.env_type == 'robotwin_tshape':
            ...
            videos_high = videos[0] / 255.0 * 2.0 - 1.0
            videos_wrist = videos[1] / 255.0 * 2.0 - 1.0
            vae_device = next(self.streaming_vae.vae.parameters()).device
            enc_out_high = self.streaming_vae.encode_chunk(
                videos_high.to(vae_device).to(self.dtype))
    ...
    def _reset(self, prompt=None):
        ...
        if self.env_type == 'robotwin_tshape':
            ...
            self.latent_height, self.latent_width = (
                self.height // 16 * len(self.job_config.obs_cam_keys),
                self.width // 16,
            )
            self.streaming_vae_half.clear_cache()
        else:
        ...
        self.actions_q01 = torch.tensor(self.job_config.norm_stat['q01']+[0.] * 23,
                                        dtype=torch.float32).reshape(-1, 1, 1)
        self.actions_q99 = torch.tensor(self.job_config.norm_stat['q99']+[1.] * 23,
                                        dtype=torch.float32).reshape(-1, 1, 1)
        ...
        self.exp_save_root = os.path.join(self.save_root, 'real', self.exp_name) if self.save_root else None
...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help='Training checkpoint dir (e.g. .../checkpoints/checkpoint_step_800). '
             'Transformer is loaded from ckpt/transformer; VAE/tokenizer/text_encoder from config base.',
    )
